...
#### 一、核心定位
作为项目 AI 视觉算法负责人，我负责浮游植物识别全流程算法开发，并**为预测可视化提供空间仿真工具**。核心职责：基于公开数据集训练天气分类模型；基于贺一冉交付的标注数据集训练**三个天气专用的 DeepLabV3+ 语义分割模型**，同时训练**一个不区分天气的统一 DeepLabV3+ 模型作为对比基线**；开发推理管道，输出实测掩码与基于元数据的量化结果；**提供空间仿真脚本，供数模组（刘俊辉）调用，以生成未来浮游植物分布图**。模型本身绝不直接输出面积，只做像素级分割。
...
2. 分割模型训练：
   ...
   - **【新增】额外使用全部标注数据（不区分天气）训练一个统一的 DeepLabV3+ 模型，作为对比基线，用于验证天气自适应策略的有效性。**
...
8. **【新增】对比实验与评估报告**：完成四种分割模型（统一模型 + 三种天气专属模型）的训练后，在相同测试标准下计算 mIoU、Accuracy（以及可选 F1），产出两个对比表格，附于模型评估报告，并给出结论。
...
- **【新增】对比模型脚本位置**：
  - `models/deeplabv3plus/combined/train.py`
- 数据集划分：
  - 专属模型：每个训练脚本读取 `../../../data/03_labeled/` 下对应天气的图像与掩码（通过文件名过滤）。
  - **对比模型：读取全部天气的图像与掩码，混合训练。训练脚本需同时保存评估指标（mIoU, Accuracy, F1）至日志或模型目录下的 `metrics.json`，以便汇总。**
- 模型保存路径：
  - 专属模型：各天气目录下的 `best_model.pth`
  - **对比模型**：`models/deeplabv3plus/combined/best_model.pth`
...
**5. 【新增】对比模型评估与指标统计**
- 在各自训练脚本中，已完成对验证集/测试集的指标计算（mIoU, Accuracy, F1）。
- 汇总四个模型的最佳指标，形成两份表格：
  - **表格1（宏观对比）**：统一模型（Combined）的指标 vs. 三种天气专属模型的指标平均值。
  - **表格2（分天气细节）**：晴天、阴天、雾天专属模型各自在对应测试集上的指标。
- 表格需写入模型评估报告（参见交付物第8项），并附简要分析，明确回答：**天气自适应策略是否带来整体精度提升，以及不同天气下模型表现的差异是否明显**。
...
- **本人（吴天宇）**：公开数据集 → 训练天气分类模型；标注数据集 → 训练分割模型（含统一对比模型）；推理 + 量化 + 仿真脚本 + 对比评估 → 输出至 `outputs/`
...
2. 三个天气专属 DeepLabV3+ 分割模型权重（`models/deeplabv3plus/{weather}/best_model.pth`）及训练脚本。
3. **【新增】一个统一 DeepLabV3+ 对比模型权重（`models/deeplabv3plus/combined/best_model.pth`）及训练脚本。**
4. 总推理脚本 `models/predict_pipeline.py`。
5. 空间仿真脚本 `utils/simulation/generate_prediction_mask.py` 及详细使用说明。
6. 正射校正脚本（`utils/preprocess/orthorectify.py`）及使用说明。
7. 实测分割掩码图（`outputs/masks/`）与量化数据（`outputs/quantifications/`）。
8. `outputs/` 数据格式规范文档。
9. **模型评估报告（含四个模型的对比表格、天气自适应必要性分析）**。